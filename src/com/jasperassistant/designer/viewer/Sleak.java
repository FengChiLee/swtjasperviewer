...
/**
 * SWT resources leak detection tool See:
...
//				regions++;
	...
	void refreshDifference() {
		...
		Object[] newObjects = findObjects(info);
	...
	void refreshObject() {
		...
		if (check.getSelection()) {
			...
			} else if (objects[index] instanceof Control) {
				Control c = (Control) objects[index];
				StringBuffer buf = new StringBuffer();
				while (c != null) {
					buf.insert(0, objectName(c));
					c = c.getParent();
					if (c != null)
						buf.insert(0, '/');
				}
