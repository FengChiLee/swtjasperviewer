 ...
 */
public class ExportAsMultiXlsxAction extends AbstractExportAction {
	 ...
	 */
	public ExportAsMultiXlsxAction(IReportViewer viewer) {
		super(viewer);

		setText(Messages.getString("ExportAsMultiXlsxAction.label")); //$NON-NLS-1$
		setToolTipText(Messages.getString("ExportAsMultiXlsxAction.tooltip")); //$NON-NLS-1$
		setImageDescriptor(ICON);
		setDisabledImageDescriptor(DISABLED_ICON);

		setFileExtensions(new String[] { "*.xlsx" }); //$NON-NLS-1$
		setFilterNames(new String[] { Messages.getString("ExportAsMultiXlsxAction.filterName") }); //$NON-NLS-1$
		setDefaultFileExtension("xlsx"); //$NON-NLS-1$
	}
	
    public void setIPosExportXlsHandler(IPosExportXlsHandler posExportSingleXlsHandler) {

        this.posExportXlsHandler = posExportSingleXlsHandler;
    }
	...
	protected void exportWithProgress(File file, JRExportProgressMonitor monitor)
        ...
        if(posExportXlsHandler != null) {
            ...
            posExportXlsHandler.removeParameter("IS_IGNORE_PAGINATION");
