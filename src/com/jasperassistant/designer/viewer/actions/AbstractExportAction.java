...
public abstract class AbstractExportAction extends AbstractReportViewerAction {
	...
	public AbstractExportAction(IReportViewer viewer) {
		super(viewer);
		if(this.getReportViewer() != null) {
			fileName = this.getReportViewer().getExportFileName();
		}
	}
	...
	public void run() {
		...
		String defaultFileName = dialog.getFileName();
		String dExt = getFileExtension(defaultFileName);
		if(dExt == null && defaultFileExtension != null) {
			defaultFileName += "." + defaultFileExtension;
		}
		
		File dFile = new File(dialog.getFilterPath(), defaultFileName);
		if(dFile.exists()) {
			dialog.setFileName(fileName + "(1)");
		}
		...
		if (filePath != null) {
			...
			if(file.exists()) {
				...
				mb.setMessage(MessageFormat.format(
						Messages.getString("AbstractExportAction.exportFileExists"),
			...
			try {
				...
				String title = Messages.getString("AbstractExportAction.exportFileFailed");
